/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   utoa.c                                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: amghazar <amghazar@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/02/19 18:45:22 by amghazar          #+#    #+#             */
/*   Updated: 2026/02/27 18:35:18 by amghazar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../libft/libft.h"
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>

static size_t	get_digits_number(unsigned int n)
{
	unsigned short	digits_number;
	unsigned int	i;

	digits_number = 0;
	i = n;
	while (i != 0)
	{
		i /= 10;
		digits_number++;
	}
	return (digits_number);
}

static char	*get_num_str(unsigned int n, size_t digits_number)
{
	char				*int_str;
	unsigned int		i;
	int					j;

	i = n;
	int_str = malloc(digits_number + 1);
	if (int_str == NULL)
		return (NULL);
	j = digits_number - 1;
	while (j >= 0)
	{
		int_str[j] = i % 10 + '0';
		i /= 10;
		j--;
	}
	int_str[digits_number] = '\0';
	return (int_str);
}

char	*utoa(unsigned int n)
{
	size_t	digits_number;

	if (n == 0)
		return (ft_strdup("0"));
	digits_number = get_digits_number(n);
	return (get_num_str(n, digits_number));
}
